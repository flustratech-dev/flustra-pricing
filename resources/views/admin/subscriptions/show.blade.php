...
@section('page_title', 'Rincian Langganan')
            ...
            <div class="card-body">
                <ul class="list-unstyled mb-0 d-flex flex-column gap-3">
                    <li class="d-flex justify-content-between">
                        <span>Pengguna:</span>
                        <strong class="text-dark">{{ $subscription->user->name }} ({{ $subscription->user->email }})</strong>
                    </li>
                    <li class="d-flex justify-content-between">
                        <span>Nama Paket:</span>
                        <strong class="text-dark">{{ $subscription->plan->name }}</strong>
                    </li>
                    <li class="d-flex justify-content-between">
                        <span>Status:</span>
                    ...
                    <li class="d-flex justify-content-between">
                        ...
                        <strong class="text-dark">{{ ucfirst($subscription->billing_cycle) }}</strong>
                    </li>
                    <li class="d-flex justify-content-between">
                        <span>Biaya Langganan:</span>
                        <strong class="text-dark">{{ $subscription->getFormattedPrice() }}</strong>
                    </li>
                    <li class="d-flex justify-content-between">
                        <span>Dimulai:</span>
                        <strong class="text-dark">{{ $subscription->started_at?->format('d M Y H:i') }}</strong>
                    </li>
                    <li class="d-flex justify-content-between">
                        <span>Berakhir:</span>
                        <strong class="text-dark">{{ $subscription->ended_at?->format('d M Y H:i') ?: '—' }}</strong>
                    </li>
                    <li class="d-flex justify-content-between">
                        <span>Auto-Perpanjang:</span>
                        <strong class="text-dark">{{ $subscription->auto_renew ? 'Ya' : 'Tidak' }}</strong>
                    </li>
                    <li class="d-flex justify-content-between">
                        <span>External ID:</span>
                        <strong class="text-dark text-uppercase" style="font-size: 0.85rem;">{{ $subscription->external_subscription_id ?: 'LOCAL-' . $subscription->id }}</strong>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body d-flex flex-column gap-3">
                ...
                <form action="{{ route('admin.subscriptions.upgrade', $subscription) }}" method="POST" class="row g-2 align-items-end p-3 rounded-4" style="background: rgba(255, 255, 255, 0.01); border: 1px solid var(--border-color);">
                    ...
                    <div class="col-4">
                        <button type="submit" class="btn btn-primary btn-sm w-100 py-2">Upgrade</button>
                    </div>
                </form>
                ...
                <form action="{{ route('admin.subscriptions.downgrade', $subscription) }}" method="POST" class="row g-2 align-items-end p-3 rounded-4" style="background: rgba(255, 255, 255, 0.01); border: 1px solid var(--border-color);">
                    ...
                    <div class="col-4">
                        <button type="submit" class="btn btn-outline-secondary btn-sm w-100 py-2 text-warning border-warning border-opacity-10">Downgrade</button>
                    </div>
                </form>
                ...
                <div class="d-flex gap-2">
                    <form action="{{ route('admin.subscriptions.renew', $subscription) }}" method="POST" class="flex-grow-1 m-0">
                        @csrf
                        <button type="submit" class="btn btn-outline-secondary w-100 py-3 text-success border-success border-opacity-20" style="background: rgba(16, 185, 129, 0.05);">
                    ...
                    <form action="{{ route('admin.subscriptions.cancel', $subscription) }}" method="POST" class="flex-grow-1 m-0" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan langganan pengguna ini secara paksa?');">
                        ...
                        <button type="submit" class="btn btn-outline-secondary w-100 py-3 text-danger border-danger border-opacity-20" style="background: rgba(239, 68, 68, 0.05);">
                    ...
                    @forelse($subscription->invoices as $inv)
                        <li class="d-flex justify-content-between align-items-center p-2 rounded-3" style="background: rgba(255, 255, 255, 0.02); border: 1px solid var(--border-color);">
                            ...
                            <div class="text-end">
                                ...
                                <br>
                                <strong class="text-dark">{{ $inv->getFormattedAmount() }}</strong>
                            </div>
                        </li>
                    @empty
                ...
                <ul class="list-unstyled mb-0 d-flex flex-column gap-3">
                    @forelse($subscription->logs as $log)
                        <li class="p-2 rounded-3" style="background: rgba(255, 255, 255, 0.01); border: 1px solid var(--border-color);">
                            ...
                            <p class="text-dark small mb-0">{{ $log->notes }}</p>
                        </li>
                    @empty
