<x-guest-layout>
    <div class="w-full max-w-5xl bg-cream-100 rounded-[2.5rem] shadow-[0_25px_60px_-15px_rgba(139,94,60,0.15)] overflow-hidden flex flex-col md:flex-row min-h-[600px] border border-cream-200/50">
        ...
        <div class="w-full md:w-1/2 p-8 sm:p-12 md:p-16 flex flex-col justify-center bg-cream-50">
            ...
            @if (session('status'))
                <div class="mb-6 p-4 rounded-xl bg-cream-100/80 border border-cream-200 text-cream-800 text-xs font-sans">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <!-- Email Address -->
                <div>
                    <label for="email" class="block text-[11px] font-bold text-cream-800 tracking-[0.1em] mb-2">EMAIL</label>
                    <div class="relative flex items-center">
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username" 
                            ...
                            class="w-full bg-cream-100/50 border border-cream-200 rounded-xl py-3.5 pl-11 pr-4 text-cream-900 text-sm placeholder-cream-400 focus:outline-none focus:border-brown focus:ring-2 focus:ring-brown/20 transition-all duration-200 shadow-sm" />
                    </div>
                    @if ($errors->has('email'))
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        <label for="password" class="text-[11px] font-bold text-cream-800 tracking-[0.1em]">KATA SANDI</label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-[11px] font-semibold text-brown hover:text-brown/80 transition-colors font-sans">
                                Lupa Kata Sandi?
                            </a>
                        @endif
                    </div>
                    <div class="relative flex items-center">
                        ...
                        <input id="password" type="password" name="password" required autocomplete="current-password" 
                            placeholder="••••••••" 
                            class="w-full bg-cream-100/50 border border-cream-200 rounded-xl py-3.5 pl-11 pr-11 text-cream-900 text-sm placeholder-cream-400 focus:outline-none focus:border-brown focus:ring-2 focus:ring-brown/20 transition-all duration-200 shadow-sm" />
                        
                        <!-- Toggle Show Password -->
                        <button type="button" onclick="togglePasswordVisibility('password', 'eye-icon')" class="absolute right-4 text-cream-700 hover:text-brown transition-colors focus:outline-none">
                ...
                <div class="flex items-start">
                    ...
                    <label for="remember_me" class="ml-3 text-xs text-cream-800 leading-relaxed cursor-pointer font-sans">
                ...
                <div>
                    <button type="submit" 
                        class="w-full bg-brown hover:bg-brown/80 text-white font-semibold py-3.5 px-4 rounded-xl shadow-[0_4px_12px_rgba(139,94,60,0.25)] hover:shadow-[0_6px_16px_rgba(139,94,60,0.35)] focus:outline-none focus:ring-2 focus:ring-brown/50 active:scale-[0.98] transition-all duration-150 text-center text-sm font-sans">
                        Masuk
                    </button>
                </div>
            </form>
            ...
            <!-- Bottom Navigation -->
            <p class="text-center text-xs sm:text-sm text-cream-800 font-sans mt-8">
                Belum memiliki akun? <a href="{{ route('register') }}" class="font-bold text-brown hover:text-brown/80 hover:underline transition-all">Daftar Sekarang</a>
            </p>
